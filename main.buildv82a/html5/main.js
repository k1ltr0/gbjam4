...
c_TileMapCollider.prototype.p_GetTileID=function(t_cx,t_cy){
	...
	if(t_index<this.m_tile_layer.m_data.length-1){
...
c_SpaceShooterCannon.prototype.p_Update=function(){
	...
	while(t_.p_HasNext()){
		...
		t_b.m_Position.p_X(t_2.p_X2()+(this.m_speed)*c_Time.m_DeltaSecs());
		t_b.m_current_live_time+=c_Time.m_DeltaSecs();
		if(t_b.m_max_live_time*(1+this.m_level)<=t_b.m_current_live_time){
			t_b.p_Destroy2();
			this.m_discard_list.p_AddLast4(t_b);
		}
	}
	var t_3=this.m_discard_list.p_ObjectEnumerator();
	while(t_3.p_HasNext()){
...
c_SpaceShooterCannon.prototype.p_Shot=function(){
	...
	this.m_bullets.p_AddLast4(t_img);
	this.m_camera_fx.p_Shake(100,0.2*(this.m_level+1),0.2*(this.m_level+1));
}
...
c_CameraFX.prototype.p_Shake=function(t_time,t_force_x,t_force_y){
	this.m_timer=0;
	this.m_time=t_time;
	this.m_force_x=t_force_x;
	this.m_force_y=t_force_y;
	this.m_state=1;
	return 0;
}
...
c_EnemyCannon.prototype.p_Create=function(){
	this.m_bullets=c_List7.m_new.call(new c_List7);
	this.m_discard_list=c_List7.m_new.call(new c_List7);
}
...
c_EnemyCannon.prototype.p_Shot2=function(t_ox,t_oy,t_dx,t_dy){
	...
	this.m_bullets.p_AddLast6(t_bullet);
}
c_EnemyCannon.prototype.p_Update=function(){
	...
	while(t_.p_HasNext()){
		...
		if(t_b.m_max_live_time<=t_b.m_current_live_time){
			t_b.p_Destroy2();
			this.m_discard_list.p_AddLast6(t_b);
		}
	}
...
c_EnemyWave.m_new=function(t_position){
	...
	this.m_position=t_position;
	this.m_initial_y=((this.m_position.p_Y2())|0);
...
c_EnemyWave.prototype.p_Update=function(){
	...
	this.m_position.p_Y((this.m_initial_y)+Math.sin((this.m_position.p_X2() % 45.0*8.0)*D2R)*7.0);
}
